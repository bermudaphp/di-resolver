...
final class ParameterResolver implements \Countable, IteratorAggregate
{
     ...
     * Constructor.
     *
     * @param iterable<ParameterResolverInterface> $resolvers An iterable of instances that implement ParameterResolverInterface.
     */
    ...
    {
        if ($resolver instanceof ParameterResolverInterface) $resolver = $resolver::class;
        foreach ($this->resolvers as $rslvr) if ($rslvr::class == $resolver) return true;
    ...
    public function resolveParameter(ReflectionParameter $parameter, array $providedParameters = [], array $resolvedParameters = []): array
    {
        ...
        throw ParameterResolutionException::create($parameter, $providedParameters, $resolvedParameters);
    }
