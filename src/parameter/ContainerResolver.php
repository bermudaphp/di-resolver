        ...
        foreach ($path as $i => $key) {
            try {
                ...
                if (is_array($entry) || $entry instanceof \ArrayAccess) {
                    ...
                    throw new ParameterResolutionException($parameter, $providedParameters, $resolvedParameters,"Undefined config key: $pathString");
                } else {
                    $pathString = implode(' → ', array_slice($path, 0, $i));
                    throw new ParameterResolutionException($parameter, $providedParameters, $resolvedParameters, "Config value at path '$pathString' is not accessible");
                }
            }
        }
