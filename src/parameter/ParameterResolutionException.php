...
class ParameterResolutionException extends \RuntimeException implements ParameterResolutionExceptionInterface
{
    ...
    {
        $self = new self($parameter, 'Argument #'.$parameter->getPosition().' ($'.$parameter->getName().') must be of type ' . $parameter->getType() . ', given ' . Type::gettype($entry, Type::FLAG_OBJECT_AS_CLASS));
